...
class Phone {
	...
	public static function phone_decode($phone = '', $convert = true, $trim = true)
	{
	    if (empty($phone)) {
	        return '';
	    }
	    ...
	    foreach (self::$phoneCodes as $countryCode=>$data)
	    {
	        ...
	        if (substr($phone, 0, $codeLen)==$countryCode)
	        {
	            ...
	            return '+'.$countryCode.' ('.$cityCode.') '.self::phoneBlocks($phone);
	        }
	    }
	...
	{
		$text = urlencode(mb_convert_encoding($text, 'HTML-ENTITIES', 'UTF-8'));
		//.urlencode(mb_convert_encoding('asas '.$order->id, 'HTML-ENTITIES', "UTF-8"))
		file('http://gatesms.ru/sender.php?lg=absolute&pw=alenka&fl=s&to='.$number.'&mg='.$text);
	}
